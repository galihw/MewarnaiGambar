function Draw6(no,div1,canv1,div2,canv2,nourut){
	// untuk mendapatkan huruf dan koordinatnya
	...
	// untuk mendapatkan huruf dan koordinatnya
	//var nn = RandomAngkaAtoB(2,8);
	var nn = no+2;
	var ss = datakata6(nn);
	//console.log(ss[0]);//untuk kata
	//console.log(ss[1]);//untuk huruf kepake A=2, B=1, dll
	//console.log(ss[2]);//untuk koordinat per huruf
	// untuk mendapatkan huruf dan koordinatnya
	...
	if(nourut!==0){
		...
		divisi1.innerHTML = " Bagian 6 (Kata-Warna) - Soal "+no;
	...
	async function drawAll() {
		...
		*/
		...
		var max = arimg.length-ss[1].length
		...
		loadedImages.forEach((img, index) => {
			...
			if(index==0){
				...
				for(var i=0;i<ss[0].length;i++){
					ctx1.fillText(ss[0][i],ss[2][i][0], ss[2][i][1]);
				}
				ctx2.save();
			}else if(index<maxlength){
				//console.log("- "+ss[0][index-1]);
				var nopalet = 0;
				for(var i=0;i<ss[1].length;i++){
					if(ss[0][index-1]==ss[1][i][0]){
						nopalet=i;
						break;
					}
					//console.log(ss[1][i][0]);
				}
				//ctx2.filter = arcolor[index][0];
				ctx2.filter = arcolor[nopalet+1][0];
				ctx2.drawImage(img, 0, 0);
				ctx2.filter = "none";
				ctx2.save();
			}else if(index==maxlength){
				...
				ctx1.fillText(ss[1][index-maxlength-1][0],530, 100+30*(index-maxlength));
				ctx1.textAlign = "left";
